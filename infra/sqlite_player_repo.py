...
class SQLitePlayerRepository(PlayerRepositoryPort):
    ...
    def init_schema(self) -> None:
        ...
        # players 表列迁移（之前已加过）
        cols = {
            r[1] for r in self._conn.execute("PRAGMA table_info(players)").fetchall()
        }
        if "draw_count" not in cols:
            self._conn.execute(
                "ALTER TABLE players ADD COLUMN draw_count INTEGER DEFAULT 0;"
            )
        ...
        if "last_move_at" not in cols:
            self._conn.execute(
                "ALTER TABLE players ADD COLUMN last_move_at INTEGER DEFAULT 0;"
            )
        self._conn.commit()
    ...
    def get_last_move_at(self, user_id: str) -> int:
        cur = self._conn.execute(
            "SELECT last_move_at FROM players WHERE user_id=?", (user_id,)
        )
        r = cur.fetchone()
    ...
    def upsert_player(self, p: Player) -> None:
        ...
        """,
            (
                ...
                p.barracks_level,
                p.draw_count,
                getattr(p, "base_city", None),
                getattr(p, "base_x", None),
                getattr(p, "base_y", None),
                getattr(p, "last_move_at", 0),
            ),
        )
        self._conn.commit()
    ...
    def list_owned_char_names(self, user_id: str):
        cur = self._conn.execute(
            "SELECT name FROM player_chars WHERE user_id=?", (user_id,)
        )
        return [r[0] for r in cur.fetchall()]

    def has_char(self, user_id: str, name: str) -> bool:
        cur = self._conn.execute(
            "SELECT 1 FROM player_chars WHERE user_id=? AND name=? LIMIT 1",
            (user_id, name),
        )
    ...
    def get_char_level(self, user_id: str, name: str):
        cur = self._conn.execute(
            "SELECT level FROM player_chars WHERE user_id=? AND name=?", (user_id, name)
        )
        r = cur.fetchone()
    ...
    def set_char_level(self, user_id: str, name: str, level: int):
        self._conn.execute(
            "UPDATE player_chars SET level=? WHERE user_id=? AND name=?",
            (level, user_id, name),
        )
        self._conn.commit()
    ...
    def get_team_soldiers(self, user_id: str, team_no: int) -> int:
        cur = self._conn.execute(
            "SELECT soldiers FROM teams WHERE user_id=? AND team_no=?",
            (user_id, team_no),
        )
        r = cur.fetchone()
    ...
    def set_team_soldiers(self, user_id: str, team_no: int, soldiers: int):
        self._conn.execute(
            "UPDATE teams SET soldiers=? WHERE user_id=? AND team_no=?",
            (soldiers, user_id, team_no),
        )
        self._conn.commit()
    ...
    def get_alliance_by_name(self, name: str):
        cur = self._conn.execute(
            "SELECT id,name,leader_user_id,created_at FROM alliances WHERE name=?",
            (name,),
        )
        r = cur.fetchone()
    ...
    def count_alliance_members(self, alliance_id: int) -> int:
        cur = self._conn.execute(
            "SELECT COUNT(1) FROM alliance_members WHERE alliance_id=?", (alliance_id,)
        )
    ...
    def get_active_siege_by_alliance(self, alliance_id: int):
        ...
        return (
            ...
            if not r
                ...
                zip([c[1] for c in self._conn.execute("PRAGMA table_info(sieges)")], r)
            )
        )
    ...
    def update_siege_state(self, siege_id: int, state: str, result: str | None):
        self._conn.execute(
            "UPDATE sieges SET state=?, result=? WHERE id=?", (state, result, siege_id)
        )
        self._conn.commit()

    # === 攻城：参战队列 ===
    def add_siege_participant(
        self,
        siege_id: int,
        user_id: str,
        from_city: str,
