...
PROVINCE_LABEL_POS: Dict[str, Tuple[int, int]] = {
    ...
    "扬": (610, 370), "荆": (235, 375), "益": (140, 485),
}

LABEL_ANCHOR: Dict[str, str] = {
    "长安":"E","潼关":"N","洛阳":"N","许昌":"E","陈留":"N","汝南":"E",
    "邺":"N","南皮":"S","鄄城":"S","东阿":"N","临淄":"S","北海":"S",
    "彭城":"S","下邳":"E","建业":"W","会稽":"W","襄阳":"E","江陵":"E",
    "汉中":"W","成都":"N","江州":"N",
}

def _city_title(c: City) -> str:
    kind = "州府" if c.capital else ("城市" if c.ntype=="CITY" else ("关隘" if c.ntype=="PASS" else "资源镇"))
    return f"{c.name}（{c.province}州，{kind}）"

def _pick_icon(city: City, assets: Dict) -> Optional[str]:
    if assets.get("cities", {}).get(city.name): return assets["cities"][city.name]
    if city.ntype == "PASS" and assets.get("defaults", {}).get("PASS"): return assets["defaults"]["PASS"]
    if city.ntype == "CITY" and assets.get("defaults", {}).get("CITY"): return assets["defaults"]["CITY"]
    if city.ntype == "RESOURCE" and assets.get("defaults", {}).get("RESOURCE"): return assets["defaults"]["RESOURCE"]
    return assets.get("defaults", {}).get("DEFAULT")

def _label_attrs(x: int, y: int, anchor: str):
    if anchor == "W": return x - LABEL_OFFSET, y + 5, "end", 0
    if anchor == "N": return x, y - (LABEL_OFFSET + 2), "middle", 0
    if anchor == "S": return x, y + (LABEL_OFFSET + FONT_SIZE - 6), "middle", 0
...
def build_map_html(graph: MapGraph, get_progress, assets: Dict):
    ...
    if assets.get("bg"):
        ...
        layers_bg.append(f'<rect x="0" y="0" width="{CANVAS_W}" height="{CANVAS_H}" fill="#f7f7f3"></rect>')
